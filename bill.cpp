...
class bill
{
	...
	public:
		...
		{
			cout<<"\n "<<d.dd<<"/"<<d.mm<<"/"<<d.yy<<"    "<<billnumber<<"\t    "<<tempamount<<setw(10)<<amount<<"\t "<<partyname<<"\t ";
		}
...
void payment::delete_payment(int mode,char *m,struct date da,int bn)
{
	...
	while(fin.read((char*)this,sizeof(*this)))
	{
		if((strcmp(m,this->mono)==0)&&(da.dd==d.dd&&da.mm==d.mm&&da.yy==d.yy))
		{
			...
			{
				gstbill b1;
				b1.update_gstbilldata(mode,this->billnumber,this->amount,0);
			}
	...
	product p;
		...
		if(mode==1)
		{
			...
			if(a==1)
			{
				...
				b.read_billdata(3,mono);
			}
			else if(a==2)
			{
				cout<<"\n Enter A Bill Number : ";
				cin>>k;
				b.search_bill(mode,k,mono);
				b.display_basicbilldata();
				b.display_companydata();
				b.display_partydata();
				p.structure_productdata();
		...
		else if(mode==2||mode==3)
		{
			...
			if(a==1)
			{
				...
				b1.read_gstbilldata(mode,3,mono);
			}
			else if(a==2)
			{
				cout<<"\n Enter A Bill Number : ";
				cin>>k;
				b1.search_bill(mode,k,0);
...
void display_partydata(int mode)
{
	...
	else if(mode==2||mode==3)
	{
		...
		cout<<"\n\n______________''Business Dill''_________________________________________________";
		cout<<"\n Date\t  BillNo   Amount ";
		a=b1.read_gstbilldata(mode,2,mono);
		cout<<"\n_______________________________________________________________________________";
		cout<<"\n Total Amount = "<<a;
		cout<<"\n_______________________________________________________________________________";
		cout<<endl;
...
void deletebill(int mode)
{
	...
	{
		bill b;
		b.delete_billdata(bn);
	}
	else if(mode==2)
	{
		gstbill b;
		b.delete_billdata(mode,bn);
	}
}
		...
		case 1:
			{
				...
				else if(mode==2||mode==3)
				{
					...
					gstbill b1;
					b1.update_gstbilldata(mode,bn,gstsum,sum);
				}
				break;	
			}
		case 2:
			{
				...
				{
					gstbill b1;
					b1.update_gstbilldata(mode,bn,0,0);
				}
				break;
			}	
		case 3:
